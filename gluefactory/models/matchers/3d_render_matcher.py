from ...geometry.gt_generation import (
    gt_line_matches_from_3d_render,
    gt_line_matches_from_homography,
    gt_matches_from_3d_render,
    gt_matches_from_homography
)
from ..base_model import BaseModel


class RendererMatcher(BaseModel):
    default_conf = {
    ...
    def _forward(self, data):
        result = {}
        if self.conf.use_points:
            result = gt_matches_from_3d_render(
                data["keypoints0"],
                data["keypoints1"],
                data["uv_data"],
                data["view0"]["image"],
                data["view1"]["image"],
                data["src_original_image_size"],
                data["src_image_size"],
                data["src_scales"],
                pos_th=self.conf.th_positive,
                neg_th=self.conf.th_negative,
            )
            # dict_keys(['assignment', 'reward', 'matches0', 'matches1', 'matching_scores0', 'matching_scores1', 'proj_0to1', 'proj_1to0'])
            ...
            # )
        if self.conf.use_lines:
            line_assignment, line_m0, line_m1 = gt_line_matches_from_3d_render(
                data["lines0"],
                data["lines1"],
                data["valid_lines0"],
                data["valid_lines1"],
